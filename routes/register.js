...
router.post("/", async (req, res) => {
  ...
  if (user) {
    error.phone_number = "Mobile Number already Exist";
    return res.status(400).send({ errors });
  }

  user = new StudentList(
    _.pick(req.body, [
      "first_name",
      "last_name",
      "phone_number",
  ...
  axios
    .get(
      "https://apps.learncab.com/payment/sms.php?mobile_no=" +
        ...
        ",\n" +
        vcode +
        " is your OTP for LearnCab account verification."
    )
    .then(res => {});
  ...
  axios
    .get(
      "https://apps.learncab.com/payment/create_hubspot_contact.php?first_name=" +
        req.body.first_name +
        "&last_name=" +
        req.body.last_name +
        "&email=" +
...
router.post("/verifyotp", async (req, res) => {
  ...
  let user = await StudentList.findOne({
  ...
  let exp_date = new Date(
    new Date().getTime() + nuOfDays * 24 * 60 * 60 * 1000
  );
